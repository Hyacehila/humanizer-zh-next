...
1. **通读原文。** 判断文本类型和目标读者，标出所有命中的模式。
2. **写初稿。** 先处理空泛、虚高、模板化的地方，再处理句式、标点、标题和填充词。检查初稿：朗读是否通顺，句长有没有变化，是不是优先用了具体细节和简单判断句（能用“是”就用“是”），语域是否合适，信息是否与原文等量覆盖。
3. **自问一句：“是什么让下面这段一眼就像 AI 生成的？”** 用一两条要点简短列出初稿里仍残留的痕迹。
4. **改出终稿，** 消除这些痕迹。交付前再扫一遍：事实是否保留，语气是否自然，有没有改得过度口语化，破折号是不是成串出现，短句是不是被堆成戏剧化的节奏（见 #14、#31）。

交付前可选用 1-10 分快速检查五个维度，总分低于 35 分时再改一轮：
